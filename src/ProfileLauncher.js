...
export class ProfileLauncher {
    ...
    /**
     * Run comprehensive authenticity test for an active session
     * @param {string} sessionId - Session ID
     * @param {Object} options - Test options
     * @returns {Object} Authenticity test results
     */
    async testAuthenticity(sessionId, options = {}) {
        const browserInfo = this.activeBrowsers.get(sessionId);
        if (!browserInfo) {
            throw new Error(`No active browser found for session: ${sessionId}`);
        }
        ...
        console.log('🔍 Running comprehensive authenticity test for active session...');
        
        const results = await this.fingerprintTester.runAuthenticityTest(page, {
            includeFingerprint: true,
            includeBotDetection: true,
            saveResults: true,
            outputPath: path.join(browserInfo.profile.userDataDir, `authenticity-test-${Date.now()}.json`),
            ...options
        });

        console.log(this.fingerprintTester.generateAuthenticityReport(results));
        return results;
    }

    /**
     * Compare authenticity between different stealth configurations
     * @param {string} profileId - Profile ID to test
     * @param {Array} presets - Array of stealth presets to compare
     * @returns {Object} Comparison results
     */
    async compareAuthenticityAcrossPresets(profileId, presets = ['minimal', 'balanced', 'maximum']) {
        console.log('🔬 Comparing authenticity across stealth presets...');
        ...
        for (const preset of presets) {
            ...
            try {
                // Launch with this preset
                const session = await this.launchProfile(profileId, {
                    stealth: true,
                ...
                const authenticityResult = await this.testAuthenticity(session.sessionId, {
                    saveResults: false // Don't save individual results
                });

                results.presetResults[preset] = {
                    score: authenticityResult.scores.combined,
                    authenticity: authenticityResult.overallAuthenticity,
                    recommendations: authenticityResult.recommendations.slice(0, 3), // Top 3
                    botDetectionScore: authenticityResult.scores.botDetection
                };
        ...
        // Analyze comparison
        results.comparison = this.analyzePresetComparison(results.presetResults);
        results.recommendations = this.generatePresetRecommendations(results.presetResults, results.comparison);

        return results;
    }

    /**
     * Analyze comparison between presets
     * @param {Object} presetResults - Results for each preset
     * @returns {Object} Analysis
     */
    analyzePresetComparison(presetResults) {
        const validResults = Object.entries(presetResults).filter(([_, result]) => !result.error);
        
        if (validResults.length === 0) {
            return { error: 'No valid results to compare' };
        }

        const scores = validResults.map(([preset, result]) => ({ preset, score: result.score }));
        scores.sort((a, b) => b.score - a.score); // Sort by score descending

        return {
            bestPreset: scores[0],
            worstPreset: scores[scores.length - 1],
            scoreDifference: scores[0].score - scores[scores.length - 1].score,
            allScores: scores,
    ...
    /**
     * Generate recommendations based on preset comparison
     * @param {Object} presetResults - Results for each preset
     * @param {Object} comparison - Comparison analysis
     * @returns {Array} Recommendations
     */
    generatePresetRecommendations(presetResults, comparison) {
        const recommendations = [];

        if (comparison.error) {
            recommendations.push('Unable to generate recommendations due to test errors');
            return recommendations;
        }

        const { bestPreset, worstPreset, scoreDifference } = comparison;

        recommendations.push(`🏆 Best authenticity: ${bestPreset.preset} preset (${bestPreset.score}/100)`);
        
        if (scoreDifference > 20) {
            recommendations.push(`⚠️ Significant difference between presets (${scoreDifference} points)`);
            recommendations.push(`Consider using ${bestPreset.preset} preset for better authenticity`);
        } else if (scoreDifference < 5) {
            recommendations.push(`✅ All presets perform similarly - choose based on compatibility needs`);
        }

        // Specific preset recommendations
        Object.entries(presetResults).forEach(([preset, result]) => {
            if (result.error) return;
            
            if (result.score < 50) {
                recommendations.push(`🚨 ${preset} preset shows low authenticity (${result.score}/100) - avoid for sensitive use cases`);
            } else if (result.score >= 80) {
                recommendations.push(`✅ ${preset} preset shows high authenticity (${result.score}/100) - good choice`);
            }
        });
